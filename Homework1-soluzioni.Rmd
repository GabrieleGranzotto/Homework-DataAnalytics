...
sex.conditioned.bmi <- prop.table(table(insurance$sex,insurance$catbmi), 2)
...
legend(5.5, 0.8, legend = c("Male", "Female"), fill = c("lightblue", "pink"))
prop.diff <- (table(insurance$catbmi)/1338)
...
plot(insurance$age, insurance$charges, col = smoker_col, pch = 16 + smoker, cex = 0.8, main=NULL, ylim=c(0,70000), xlim=c(16, 65))

par(fig=c(0.2, 0.8, 0, 0.2), new=T)
boxplot(insurance$age, horizontal=T, main=NULL, ylim=c(16, 65), col="lightblue", border="darkblue")

par(fig=c(0, 0.2, 0.2, 0.8), new=T)
boxplot(insurance$charges, main=NULL, ylim=c(0,70000), col="lightblue", border="darkblue")

par(fig=c(0.2, 0.8, 0.8, 1), new=T)
plot(density(insurance$age[insurance$smoker=="no"]), col=19, main="", xlim=c(16, 65))
lines(density(insurance$age[insurance$smoker=="yes"]), col=6, main=NULL)

par(fig=c(0.8, 1, 0.2, 0.8), new=T)
density.charges.no <- density(insurance$charges[insurance$smoker=="no"])
plot(density.charges.no$y, density.charges.no$x, type = "l", col=19, main="", ylim=c(0,70000))
...
```
```{r}
xx <- density(insurance$charge[insurance$smoker=="no"])
